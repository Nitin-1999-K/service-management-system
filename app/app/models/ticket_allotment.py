...
class TicketAllotment(Base):
    ...
    allocator = relationship(
        "User", foreign_keys=[allocator_id], back_populates="assigned_allotment"
    )
    engineer = relationship(
        "User", foreign_keys=[engineer_id], back_populates="received_allotment"
    )

    ticket_progress = relationship(
        "TicketProgress", back_populates="ticket_allotment", uselist=False
    )
